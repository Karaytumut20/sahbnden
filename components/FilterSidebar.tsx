...
export default function FilterSidebar() {
  ...
  // URL değiştiğinde state'i güncelle (Geri butonu vs. için)
  useEffect(() => {
    setFilters({
      city: searchParams.get('city') || '',
      minPrice: searchParams.get('minPrice') || '',
  ...
  const applyFilters = () => {
    ...
    // Filtreleri URL'e ekle, boşsa sil
    Object.entries(filters).forEach(([key, value]) => {
      if (value) params.set(key, value);
      else params.delete(key);
    });

    // Filtreleme yapıldığında sayfa 1'e dönmek iyidir
    params.delete('page');
  ...
  const clearFilters = () => {
    ...
    // Sadece kategoriyi koru, gerisini temizle
      ...
      <div className="space-y-4">
        ...
        {currentCategory === 'vasita' && (
            <>
                ...
                <div className="border-t border-gray-100 pt-3 dark:border-gray-700 animate-in fade-in">
                    <label className="text-[11px] font-bold text-gray-500 mb-1 block dark:text-gray-400">KİLOMETRE (MAX)</label>
                    <input
                        type="number"
