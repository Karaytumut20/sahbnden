...
import { getAdDetail } from '@/lib/services';
...
export default async function AdDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const ad = await getAdDetail(Number(id));

  if (!ad) return notFound();

  const formattedPrice = ad.price.toLocaleString('tr-TR');
  const sellerInfo = ad.profiles || { full_name: 'Bilinmeyen Kullanıcı', phone: '', email: '' };
    ...
    <div className="pb-10 relative">
      ...
      <div className="flex flex-col lg:flex-row gap-4 mb-20 md:mb-0">
        <div className="lg:w-[500px] shrink-0">
          <Gallery mainImage={ad.image || 'https://via.placeholder.com/800x600'} />
        </div>

        <div className="flex-1 min-w-0">
          <div className="mb-4 hidden md:block">
            <span className="block text-blue-600 font-bold text-xl">{formattedPrice} {ad.currency}</span>
            <span className="block text-gray-500 text-[12px] mt-1">{ad.city} / {ad.district}</span>
          </div>
          ...
          <div className="border-t border-gray-200">
             ...
             {ad.brand && <div className="flex justify-between py-2 border-b border-gray-100 text-sm"><span className="font-bold">Marka</span><span>{ad.brand}</span></div>}
        ...
        <div className="lg:w-[260px] shrink-0 hidden md:block">
           <SellerSidebar
             ...
             sellerPhone={sellerInfo.phone || '05XX XXX XX XX'}
             adId={ad.id}
             adTitle={ad.title}
             adImage={ad.image}
