...
import { getSavedSearches, deleteSavedSearch } from '@/lib/services';
import { Search, Trash2, Bell, Loader2 } from 'lucide-react';
...
export default function SavedSearchesPage() {
  ...
  const handleRemove = async (id: number) => {
    ...
    addToast('Arama silindi.', 'success');
    fetchData();
  };
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-sm shadow-sm p-6 dark:bg-[#1c1c1c] dark:border-gray-700">
      <div className="flex justify-between items-center mb-6 border-b border-gray-100 pb-2 dark:border-gray-700">
        <h1 className="text-xl font-bold text-[#333] dark:text-white">Favori Aramalarım ({searches.length})</h1>
      </div>
            ...
            <tbody className="text-[13px] text-[#333] dark:text-gray-200">
              {searches.map((search) => (
                <tr key={search.id} className="border-b border-gray-100 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800/50">
                  ...
                  <td className="p-3 text-gray-600 dark:text-gray-400">{new Date(search.created_at).toLocaleDateString()}</td>
                  ...
                  <td className="p-3 text-center">
                    <div className="flex justify-center gap-2">
                      <Link href={search.url} className="p-1.5 border border-gray-300 rounded hover:bg-blue-50 text-blue-600 dark:border-gray-600 dark:text-blue-400 dark:hover:bg-gray-700">
                        <Search size={14} />
                      </Link>
                      <button onClick={() => handleRemove(search.id)} className="p-1.5 border border-gray-300 rounded hover:bg-red-50 text-red-600 dark:border-gray-600 dark:text-red-400 dark:hover:bg-gray-700">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
