...
export default function PostAdCategory() {
  return (
    <div className="max-w-[800px] mx-auto py-8">
      <h1 className="text-xl font-bold text-[#333] mb-6 border-b pb-2">Adım 1: Kategori Seçimi</h1>

      <div className="bg-white border border-gray-200 shadow-sm rounded-sm">
        <ul className="divide-y divide-gray-100">
          {categories.map((cat) => (
            <li key={cat.id} className="group">
              <div className="p-4 flex items-center justify-between cursor-pointer hover:bg-blue-50 transition-colors">
              ...
              {/* Basit alt kategori gösterimi */}
              <div className="bg-gray-50 pl-12 pr-4 py-2 hidden group-hover:block border-t border-gray-100">
                <p className="text-[11px] text-gray-500 mb-2 font-semibold">ALT KATEGORİ SEÇİNİZ:</p>
                <div className="grid grid-cols-2 gap-2">
                  {cat.subs.map((sub, idx) => (
                    <Link
                      key={idx}
                      href={`/ilan-ver/detay?cat=${cat.id}&sub=${sub}`}
                      className="text-[13px] text-blue-800 hover:underline hover:text-blue-900 flex items-center"
                    >
